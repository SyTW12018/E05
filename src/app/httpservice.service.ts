...
export class HttpserviceService {
  ...
  get_post(id, callback){
    this.http.get('http://localhost:8081/getpost/' + id).subscribe(data => {
      callback(data);
    });
  }
  ...
  matricular_usuario(asignatura, contrasena_asignaturas, callback){
    var user = this.data.get_usuario();
    this.http.post('http://localhost:8081/matricular/' + asignatura, { usuario: user.get_usuario() ,password: contrasena_asignaturas }, { responseType: 'text' }).subscribe(data => {
      if(data == "OK"){
        callback(true);
      }
      else{
